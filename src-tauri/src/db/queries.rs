...
pub async fn list_projects(pool: &SqlitePool) -> Result<Vec<Project>> {
    let rows = sqlx::query(
        "SELECT id, name, description, color, icon, sort_order, created_at, updated_at FROM projects ORDER BY sort_order, created_at"
    )
...
pub async fn list_variables(pool: &SqlitePool, tier_id: &str) -> Result<Vec<Variable>> {
    let rows = sqlx::query(
        "SELECT id, tier_id, key, description, is_secret, sort_order, created_at, updated_at
         FROM variables WHERE tier_id = ? ORDER BY sort_order, created_at"
    )
...
pub async fn create_variable(pool: &SqlitePool, v: &VariableRaw) -> Result<()> {
    ...
    .bind(&v.description).bind(v.is_secret as i64).bind(v.sort_order)
...
pub async fn update_variable(pool: &SqlitePool, v: &VariableRaw) -> Result<()> {
    ...
    sqlx::query(
        "UPDATE variables SET key=?, value_enc=?, description=?, is_secret=?, updated_at=? WHERE id=?"
    )
    .bind(&v.key).bind(&v.value_enc).bind(&v.description)
    .bind(v.is_secret as i64).bind(&v.updated_at).bind(&v.id)
    .execute(pool).await
    .map_err(|e| {
        if e.to_string().contains("UNIQUE") { AppError::DuplicateKey } else { AppError::from(e) }
    })?;
...
pub async fn search_variables(pool: &SqlitePool, query: &str) -> Result<Vec<SearchResult>> {
    ...
    let rows = sqlx::query(
        "SELECT v.id as variable_id, v.key, v.description, v.tier_id,
         ...
         WHERE v.key LIKE ? OR v.description LIKE ?
